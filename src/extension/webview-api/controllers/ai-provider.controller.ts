...
export class AIProviderController extends Controller {
  ...
  async addProvider(req: Omit<AIModelProvider, 'id'>) {
    return await aiProviderDB.add(req)
  }

  async updateProvider(req: Partial<AIModelProvider> & { id: string }) {
    const { id, ...updates } = req
    return await aiProviderDB.update(id, updates)
  }

  async updateProviders(req: Partial<AIModelProvider> & { id: string }[]) {
